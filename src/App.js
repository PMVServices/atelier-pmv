...
const db = {
  ...
  async get(t,p=""){const r=await fetch(`${SUPA_URL}/rest/v1/${t}${p}`,{headers:{...this.h,"Prefer":"return=representation"}});return r.json();},
  async post(t,b){const r=await fetch(`${SUPA_URL}/rest/v1/${t}`,{method:"POST",headers:{...this.h,"Prefer":"return=representation"},body:JSON.stringify(b)});return r.json();},
  async patch(t,p,b){const r=await fetch(`${SUPA_URL}/rest/v1/${t}${p}`,{method:"PATCH",headers:{...this.h,"Prefer":"return=representation"},body:JSON.stringify(b)});return r.json();},
...
const CHAMPS = {
  ...
  "Infos électriques": [
    ...
    {id:"isol_masse",label:"Isolement enroulement/masse",type:"mesure",unite:"Ω",required:true},
    {id:"isol_uv",label:"Isolement U-V",type:"mesure",unite:"Ω",required:true},
    {id:"isol_vw",label:"Isolement V-W",type:"mesure",unite:"Ω",required:true},
    {id:"isol_wu",label:"Isolement W-U",type:"mesure",unite:"Ω",required:true},
    {id:"adx_resultat",label:"ADX mesure isol. — résultat",type:"select",options:["PASS","Douteux","Hors Tolérance"],required:true},
    {id:"adx_valeur",label:"ADX mesure isol. — valeur",type:"mesure",unite:"Ω",required:true},
  ...
  "Information rotation avant démontage": [
    ...
    {id:"int_p1_avant",label:"Intensité Phase 1",type:"mesure",unite:"A",required:true},
    {id:"int_p2_avant",label:"Intensité Phase 2",type:"mesure",unite:"A",required:true},
    {id:"int_p3_avant",label:"Intensité Phase 3",type:"mesure",unite:"A",required:true},
    {id:"vib_av_mms_avant",label:"Vibration avant — mm/s",type:"mesure",unite:"mm/s",required:true},
    {id:"vib_av_ge_avant",label:"Vibration avant — GE",type:"mesure",unite:"GE",required:true},
    {id:"vib_ar_mms_avant",label:"Vibration arrière — mm/s",type:"mesure",unite:"mm/s",required:true},
    {id:"vib_ar_ge_avant",label:"Vibration arrière — GE",type:"mesure",unite:"GE",required:true},
    {id:"int_560_p1_avant",label:"Intensité 560V — Phase 1",type:"mesure",unite:"A",required:false},
    {id:"int_560_p2_avant",label:"Intensité 560V — Phase 2",type:"mesure",unite:"A",required:false},
    {id:"int_560_p3_avant",label:"Intensité 560V — Phase 3",type:"mesure",unite:"A",required:false},
    ...
    {id:"isol_masse_hp",label:"Mesure isolement masse (suite HP)",type:"mesure",unite:"Ω",required:true,condition:{champ:"etuvage_stator",valeur:"Oui"}},
    {id:"isol_enroul_min",label:"Isolement enroulements — plus petite valeur",type:"mesure",unite:"Ω",required:true},
  ...
  "Information des essais après remontage": [
    ...
    {id:"int_p1_apres",label:"Intensité Phase 1",type:"mesure",unite:"A",required:true},
    {id:"int_p2_apres",label:"Intensité Phase 2",type:"mesure",unite:"A",required:true},
    {id:"int_p3_apres",label:"Intensité Phase 3",type:"mesure",unite:"A",required:true},
    {id:"int_560_p1_apres",label:"Intensité 560V — Phase 1",type:"mesure",unite:"A",required:false},
    {id:"int_560_p2_apres",label:"Intensité 560V — Phase 2",type:"mesure",unite:"A",required:false},
    {id:"int_560_p3_apres",label:"Intensité 560V — Phase 3",type:"mesure",unite:"A",required:false},
    {id:"vib_av_mms_apres",label:"Vibration avant — mm/s",type:"mesure",unite:"mm/s",required:true},
    {id:"vib_av_ge_apres",label:"Vibration avant — GE",type:"mesure",unite:"GE",required:true},
    {id:"vib_ar_mms_apres",label:"Vibration arrière — mm/s",type:"mesure",unite:"mm/s",required:true},
    {id:"vib_ar_ge_apres",label:"Vibration arrière — GE",type:"mesure",unite:"GE",required:true},
...
function ChampRoulement({valeur,onChange}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      <select value={selVal} onChange={e=>{if(e.target.value==="Autre")onChange("Autre:");else onChange(e.target.value);}} style={S.sel}>
        <option value="">— Sélectionner</option>
        {ROULEMENTS.map(r=><option key={r}>{r}</option>)}
      </select>
      {(selVal==="Autre"||valeur?.startsWith("Autre:"))&&(
        <input type="text" placeholder="Référence précise..." value={valeur?.replace("Autre:","")||""} onChange={e=>onChange("Autre:"+e.target.value)} style={S.inp}/>
      )}
    </div>
  );
}

function UnChamp({c,v,onChange,techs}){
  ...
  if(c.type==="technicien"){
  ...
  }else if(c.type==="roulement"){
    ctrl=<ChampRoulement valeur={val} onChange={nv=>onChange(c.id,nv)}/>;
    ...
    ctrl=<div style={{display:"flex",flexDirection:"column",gap:6}}>
      ...
      {(isAutre||val?.startsWith("Autre:"))&&c.autreTexte&&(
        <input type="text" placeholder="Préciser..." value={val?.replace("Autre:","")||""} onChange={e=>onChange(c.id,"Autre:"+e.target.value)} style={S.inp}/>
      )}
    </div>;
  }else if(c.type==="mesure"){
    ctrl=<div>
      ...
      {err&&<div style={S.alert}>⚠ Valeur élevée — vérifier (seuil: {SEUIL_DEFAUT} {c.unite})</div>}
    </div>;
  }else if(c.type==="oui_non"){
    ctrl=<div style={{display:"flex",gap:20}}>
      {["Oui","Non"].map(opt=><label key={opt} style={{display:"flex",alignItems:"center",gap:6,fontSize:14,cursor:"pointer"}}>
        <input type="radio" checked={val===opt} onChange={()=>onChange(c.id,opt)}/> {opt}
      </label>)}
    </div>;
  }else if(c.type==="date"){
    ctrl=<input type="date" value={val} onChange={e=>onChange(c.id,e.target.value)} style={S.inp}/>;
  }else if(c.type==="number"){
    ctrl=<div style={{display:"flex",alignItems:"center",gap:8}}>
...
function SectionEtape({nom,idx,actif,validees,v,nr,onChange,onNR,onValider,sessionTech,techs,saving}){
...
function PageFiche({ficheInit,sessionTech,techs,onRetour}){
  ...
  useEffect(()=>{
    ...
    db.get("fiche_valeurs",`?fiche_id=eq.${ficheInit.id}`).then(rows=>{
      ...
      const m={};rows.forEach(r=>{m[r.champ_id]=r.valeur;});
      setV(p=>({...p,...m}));
    });
  },[ficheInit?.id]);
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        {ETAPES.map((nom,i)=>(
          <SectionEtape key={nom} nom={nom} idx={i} actif={actif} validees={validees}
            ...
            onValider={()=>save(i)} sessionTech={sessionTech} techs={techs} saving={saving}
          />
        ))}
...
export default function App(){
  ...
  useEffect(()=>{
    db.get("techniciens","?actif=eq.true&order=initiales")
      .then(d=>{if(Array.isArray(d)&&d.length>0)setTechs(d.map(t=>t.initiales));})
      .catch(()=>{});
  },[]);
  ...
  return(
    <div style={S.app}>
      ...
      {page==="fiche"&&<PageFiche ficheInit={ficheOuverte} sessionTech={sessionTech||"—"} techs={techs} onRetour={()=>{setPage("accueil");setFicheOuverte(null);}}/>}
    </div>
  );
}
